...
class Encoder_z0_ODE_RNN(nn.Module):
    def __init__(self, latent_dim, input_dim, z0_diffeq_solver=None, z0_dim=None,
        ...
        self.n_sensors = kwargs.get('nsens')
        self.dim = kwargs.get('dim')
        self.enc = kwargs.get('enc_type')

        self.lstm = nn.LSTMCell(input_dim, latent_dim)
        self.lstm2 = nn.LSTMCell(input_dim, latent_dim)

        self.z0_diffeq_solver = z0_diffeq_solver
        self.z0_h_solver = kwargs.get('z0_h_solver')
        ...
        self.gcn = GNN(input_size=self.nfeats, hidden_size=32)
    ...
    def forward(self, truth, mask, time_steps, adj=None, run_backwards=True, save_info=False):
        ...
        if self.enc == 'rnn2':
            ...
            latent = self.run_odernn(truth, mask, time_steps, adj, run_backwards)
    ...
    def run_odernn(self, truth, mask, time_steps, adj, run_backwards=True):
        ...
        for i in time_points_iter:
            ...
            if adj is not None:
                xi_truth = truth[:, i, :]
                x_gcn = xi_truth.view(batch_size, 1, self.n_sensors, self.nfeats)
    ...
    def run_odernn2(self, data, time_steps, adj, run_backwards=True):
        ...
        for i in time_points_iter:
            ...
            if adj is None:
                ...
                x_truth = xi[:, :x_dim]
                x_mask = xi[:, x_dim:]
                x_gcn = x_truth if x_dim == a_dim else x_truth.view(-1, a_dim, x_dim // a_dim)
    ...
    def run_odernn3(self, data, time_steps, adj, run_backwards=True):
        ...
        for i in time_points_iter:
            ...
            h = self.z0_diffeq_solver(x=h.unsqueeze(1), adj=_h.unsqueeze(1), t=t).squeeze(1)
