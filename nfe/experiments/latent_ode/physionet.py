				...
				with open(os.path.join(dirname, txtfile)) as f:
					...
					for l in lines[1:]:
						...
						if param in self.params_dict:
							...
							# assert param in ['RecordID', 'Age', 'Gender', 'Height', 'ICUType'], 'Read unexpected param {}'.format(param)
							assert param in ['RecordID'], 'Read unexpected param {}'.format(param)
...
def variable_time_collate_fn(batch, args, device = torch.device('cpu'), data_type = 'train',
	...
	combined_tt = torch.stack([torch.cat([x[1], torch.zeros(max_len - l).to(device)], 0) for x,l in zip(batch, lens)], 0)
	combined_vals = torch.stack([torch.cat([x[2], torch.zeros(max_len - l, D).to(device)], 0) for x,l in zip(batch, lens)], 0)
	combined_mask = torch.stack([torch.cat([x[3], torch.zeros(max_len - l, D).to(device)], 0) for x,l in zip(batch, lens)], 0)
