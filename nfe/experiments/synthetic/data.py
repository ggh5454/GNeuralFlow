...
from nfe.experiments.synthetic.generate import generate
...
def load_dataset(name, args):
    n = args['n_ts']
    data_str = f'data/synth{n}' if args['dag_data'] == 0 else f'data/synth{n}g'
    DATA_DIR = Path(__file__).parents[1] / data_str
...
def get_data_loaders(name, args, batch_size):
    # Returns input_dim, n_classes=None, 3*torch.utils.data.DataLoader
    trainset, valset, testset = load_dataset(name, args).split_train_val_test()
