...
class GOB(BaseExperiment):
    ...
    def training_step(self, b, adj=None):
        _, loss, _, _, _ = self.model(times=b['times'],
                                      num_obs=b['num_obs'],
                                      X=b['X'].to(self.device),
                                      M=b['M'].to(self.device),
                                      delta_t=self.delta_t,
                                      cov=b['cov'].to(self.device),
    ...
    def validation_step(self, **kwargs):
        nll, mse = validate(self.model,
                            self.dl_val,
                            self.device,
                            self.delta_t,
                            **kwargs)
        # self.logger.info(f'val_mse={mse:.5f}')
        return {'nll': f'{nll:.4e}', 'mse': f'{mse:.4e}'}
    ...
    def eval_longer(self, args):
        ...
        nll, mse = validate(self.model, dl_val_long, self.device, self.delta_t, adj)
