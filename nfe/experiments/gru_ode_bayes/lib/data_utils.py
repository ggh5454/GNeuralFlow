...
class ITSDataset(Dataset):
    ...
    def __init__(self, in_df, validation=False, val_options=None):
        ...
        if self.validation:
            ...
            self.df_after.ID = self.df_after.ID.astype(np.int_)
            self.df_after.sort_values('Time', inplace=True)
        else:
            self.df_after = None

        self.length = self.df['ID'].nunique()
        self.df.ID = self.df.ID.astype(np.int_)
...
def collate_GOB(batch):
    ...
    df_cov = torch.Tensor(np.array([b['init_cov'] for b in batch]))
    labels = torch.tensor(np.array([b['y'] for b in batch]))
