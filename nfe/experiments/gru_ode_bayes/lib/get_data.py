...
def get_MIMIC_data(name, t_val=2.160, max_val_samples=3, return_vc=False):
    ...
    train = ITSDataset(in_df=full_data.loc[train_idx].reset_index())
    val_ = True if cuda else False
    val = ITSDataset(in_df=full_data.loc[val_idx].reset_index(), validation=val_, val_options=val_options)
    test = ITSDataset(in_df=full_data.loc[test_idx].reset_index(), validation=val_, val_options=val_options)
